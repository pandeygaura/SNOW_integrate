...
def detect_filetype(filename: str):
    ...
    if ext in ("jpg", "jpeg", "png", "bmp", "tiff", "tif"): return "image"
...
def pytesseract_extract_bytes(file_bytes, filename):
    ...
    try:
        ...
        elif ftype == "pdf" and convert_from_path:
...
def redact_labels(text):
    for label in PII_LABELS:
        ...
        text = re.sub(pattern,
                      lambda m: m.group(1) + blackout(m.group(2)),
                      text, flags=re.I)
    return text
...
def make_pdf_bytes(text):
    ...
    story = [Paragraph(l or " ", style) for l in text.splitlines()]
    doc.build(story)
    buf.seek(0)
    return buf.read()


# ------------------------ NEW FUNCTION for ServiceNow ------------------------

def process_file_bytes(file_bytes, filename, use_ai=False, language="eng"):
    """Full redaction pipeline for ServiceNow"""
...
async def process_file(
    ...
    try:
        ...
        zip_bytes = process_file_bytes(data, file.filename, use_ai, language)
...
# ------------------------ NEW SERVICENOW ROUTE ------------------------

class SNFileRequest(BaseModel):
    filename: str
    file_data: str
...
@app.post("/process_sn")
async def process_sn(req: SNFileRequest):
    try:
        # decode incoming base64
        file_bytes = base64.b64decode(req.file_data)

        # process with same logic as /process
        redacted_zip_bytes = process_file_bytes(
            file_bytes,
            req.filename,
            req.use_ai,
            req.language
        ...
        # encode outgoing base64
        redacted_zip_b64 = base64.b64encode(redacted_zip_bytes).decode("utf-8")

        return {
            "filename": f"redacted_{req.filename}.zip",
            "file_data": redacted_zip_b64
        }

    except Exception as e:
        traceback.print_exc()
        raise HTTPException(500, f"SN processing error: {str(e)}")
